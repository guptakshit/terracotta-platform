  ...
  public void test_activation_fails_single_replica_mode_enabled() {
    ...
    assertThat(
      ...
      is(throwing(instanceOf(IllegalStateException.class))
        .andMessage(allOf(
          containsString("Nodes with names: [node1] have replica-mode enabled"),
          containsString("A cluster cannot be activated if replica-mode is enabled on any node")
        )))
    );
  }
  ...
  public void test_activation_fails_replica_mode_with_other_nodes() {
    ...
    assertThat(
      ...
      is(throwing(instanceOf(MalformedClusterException.class))
        .andMessage(allOf(
          containsString("A replica-mode node with name: node1 cannot coexist with other nodes with names: [node2, node3]")
        )))
    );
  }
